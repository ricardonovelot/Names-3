...
final class VideoVolumeManager: ObservableObject {
    ...
    private static let defaultUserVolume: Float = 0.2
    private static let defaultUserVolumeWhileMusic: Float = 0.2
    ...
    private init() {
        ...
        let musicPlaying = FeatureFlags.enableAppleMusicIntegration ? MusicCenter.shared.isPlaying : false
        ...
        if FeatureFlags.enableAppleMusicIntegration {
            // Observe MusicCenter (safe facade). This does NOT create MusicPlaybackMonitor.
            MusicCenter.shared.$isPlaying
